...
import { isValidIban, formatMoney } from '@auriga-money/shared';
...
const AVATAR_COLORS = [
  ...
  'bg-auriga-accent-50 text-auriga-accent-700',
  ...
  'bg-auriga-green-50 text-auriga-black',
];
...
export default function SendPage() {
  ...
  async function handleSend() {
    ...
    try {
      ...
      const activeTransfers = JSON.parse(localStorage.getItem('auriga_active_transfers') || '[]');
      ...
      localStorage.setItem('auriga_active_transfers', JSON.stringify(activeTransfers.slice(0, 10)));
        ...
        <form onSubmit={handleReview} className="space-y-5 animate-fade-in">
          ...
          <div className="bg-white rounded-2xl border border-auriga-border/70 p-5">
            ...
            <div className="flex items-center gap-3">
              <span className="text-3xl font-semibold text-auriga-text-primary/25 select-none">€</span>
              <input
                type="number"
                placeholder="0.00"
                min="0.01"
          ...
          <div className="flex items-center gap-4 overflow-x-auto pb-1 -mx-1 px-1">
            {recentRecipients.map((r) => (
              ...
              >
                ...
                <span className={`text-[11px] text-auriga-text-secondary group-hover:text-auriga-text-primary transition-colors ${SPRING} whitespace-nowrap`}>
                  {r.name.split(' ')[0]}
                </span>
              </button>
            ))}
            <button type="button" className="flex flex-col items-center gap-1.5 group flex-shrink-0">
              ...
              <span className="text-[11px] text-auriga-text-secondary whitespace-nowrap">New</span>
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-auriga-border/70 p-5">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-auriga-text-secondary mb-1.5">
                  Pay an Auriga user by @handle
                </label>
                ...
                {resolvedHandle && (
                  <p className="mt-1.5 flex items-center gap-1.5 text-xs text-auriga-success">
          ...
          <div className={`bg-auriga-bg-subtle/50 rounded-xl px-5 py-4 space-y-2 transition-all ${SPRING}`}>
            <div className="flex justify-between text-sm">
              <span className="text-auriga-text-secondary">Speed</span>
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-auriga-success">Free</span>
            </div>
          </div>

          {/* Error */}
          {error && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-2xl bg-auriga-error/10 text-auriga-error">
      ...
      {step === 'confirm' && (
        <div className="bg-white rounded-2xl border border-auriga-border/70 overflow-hidden animate-fade-in">
          <div className="px-8 py-10 text-center">
            <p className="text-sm text-auriga-text-secondary mb-1">Sending</p>
            <p className="text-3xl sm:text-4xl font-semibold tracking-tight">
              {formatMoney(amountCents)}
            </p>
            <p className="text-lg text-auriga-text-secondary mt-2">
...
function ConfirmRow({ label, value, mono, highlight }: { label: string; value: string; mono?: boolean; highlight?: boolean }) {
  return (
    <div className="flex justify-between items-center py-3 border-b border-auriga-border/40 last:border-0">
      ...
      <span className={clsx(
        'text-sm font-medium',
        highlight && 'text-auriga-success',
        mono && 'font-mono text-xs tracking-wide',
        !highlight && !mono && 'text-auriga-text-primary',
      )}>{value}</span>
    </div>
  );
}
