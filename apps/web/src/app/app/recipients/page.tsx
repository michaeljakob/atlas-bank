...
import { isValidIban, formatIban, formatMoney } from '@atlas-bank/shared';
...
const AVATAR_COLORS = [
  { bg: 'bg-atlas-green-50', text: 'text-atlas-green-700' },
  { bg: 'bg-atlas-accent-50', text: 'text-atlas-accent-700' },
  { bg: 'bg-atlas-heather-100', text: 'text-atlas-heather-600' },
  { bg: 'bg-atlas-green-100', text: 'text-atlas-green-800' },
  { bg: 'bg-atlas-heather-200', text: 'text-atlas-text-primary' },
  { bg: 'bg-atlas-green-50', text: 'text-atlas-black' },
];
  ...
  return (
    <div className="flex h-[calc(100vh-5rem)] md:h-screen overflow-hidden">
      ...
      <div className={clsx(
        ...
        selected && 'hidden md:block md:max-w-md lg:max-w-lg md:border-r md:border-atlas-border/50',
      )}>
        ...
        <div className="relative mb-6">
          <svg className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-atlas-text-secondary/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <input
            ...
            className="w-full rounded-xl border border-atlas-border/70 bg-white py-2.5 pl-10 pr-4 text-sm placeholder:text-atlas-text-secondary/40 focus:border-atlas-border focus:outline-none transition-colors"
          />
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm text-atlas-text-secondary">No recipients found</p>
          </div>
        ) : (
          <div className="space-y-5">
            {favorites.length > 0 && (
              <section>
                <h2 className="mb-2 px-1 text-[13px] font-medium uppercase tracking-wider text-atlas-text-secondary">
                  Favorites
                </h2>
                <div className="overflow-hidden rounded-2xl border border-atlas-border/70 bg-white">
                  {favorites.map((r, i) => (
                    <RecipientRow
                      key={r.id}
                      recipient={r}
      ...
      {/* Detail panel */}
      {selected && (
        <div className="flex-1 overflow-y-auto bg-white md:bg-atlas-bg-subtle/30 animate-fade-in">
          <div className="p-4 sm:p-6 lg:p-8 max-w-xl mx-auto">
            {/* Back (mobile) */}
            <button
              onClick={() => setSelectedId(null)}
              className="md:hidden flex items-center gap-1.5 text-sm text-atlas-text-secondary mb-4 hover:text-atlas-text-primary transition-colors"
            >
            ...
            {/* Profile header */}
            <div className="bg-white rounded-2xl border border-atlas-border/70 p-6 mb-5">
              <div className="flex items-center gap-4 mb-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {selected.bank && (
                    <p className="text-sm text-atlas-text-secondary">{selected.bank}</p>
                  )}
                </div>
                <button
                  onClick={() => handleToggleFavorite(selected.id, selected.isFavorite)}
                  className={clsx('p-2 rounded-xl transition-colors', selected.isFavorite ? 'bg-atlas-green-50' : 'hover:bg-atlas-bg-subtle')}
                >
                  <svg className={clsx('w-5 h-5', selected.isFavorite ? 'text-atlas-black fill-atlas-black' : 'text-atlas-text-secondary')} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} fill={selected.isFavorite ? 'currentColor' : 'none'}>
              ...
              <div className="flex gap-3">
                ...
                <Button variant="secondary" onClick={() => handleDelete(selected.id)} className="text-atlas-error hover:bg-atlas-error/10 hover:border-atlas-error/30">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            {/* Account details */}
            <div className="bg-white rounded-2xl border border-atlas-border/70 p-5 mb-5">
                ...
                {selected.country && (() => {
                  ...
                  return (
                    <div className="flex justify-between items-center py-3 border-b border-atlas-border/40 last:border-0">
                      <span className="text-sm text-atlas-text-secondary">Country</span>
                      <span className="flex items-center gap-2 text-sm font-medium text-atlas-text-primary text-right">
            ...
            {(selected.email || selected.phone) && (
              <div className="bg-white rounded-2xl border border-atlas-border/70 p-5 mb-5">
                ...
                <div className="space-y-0">
                  {selected.email && (
                    <div className="flex justify-between items-center gap-4 py-3 border-b border-atlas-border/40 last:border-0">
                      <span className="text-sm text-atlas-text-secondary">Email</span>
                      <a href={`mailto:${selected.email}`} className="text-sm font-medium text-atlas-accent hover:underline text-right truncate">{selected.email}</a>
                    </div>
                  )}
                  {selected.phone && (
                    <div className="flex justify-between items-center gap-4 py-3 border-b border-atlas-border/40 last:border-0">
                      <span className="text-sm text-atlas-text-secondary">Phone</span>
                      <a href={`tel:${selected.phone}`} className="text-sm font-medium text-atlas-accent hover:underline text-right">{selected.phone}</a>
                    </div>
                  )}
                </div>
              </div>
            ...
            {selected.notes && (
              <div className="bg-white rounded-2xl border border-atlas-border/70 p-5 mb-5">
                ...
                <p className="text-sm text-atlas-text-primary whitespace-pre-wrap leading-relaxed">
                  {selected.notes}
                </p>
              </div>
            )}
            ...
            {recipientTransactions.length > 0 && (
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div className="bg-white rounded-2xl border border-atlas-border/70 p-4">
                  <p className="text-xs text-atlas-text-secondary mb-1">Total sent</p>
                  <p className="text-lg font-semibold text-atlas-text-primary">{formatMoney(totalSent)}</p>
                </div>
                <div className="bg-white rounded-2xl border border-atlas-border/70 p-4">
                  <p className="text-xs text-atlas-text-secondary mb-1">Total received</p>
                  <p className="text-lg font-semibold text-atlas-success">{formatMoney(totalReceived)}</p>
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-2xl border border-atlas-border/70 overflow-hidden">
              ...
              {recipientTransactions.length === 0 ? (
                <div className="px-5 pb-6 text-center">
                  <p className="text-sm text-atlas-text-secondary py-6">No transfers yet</p>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl p-8 max-w-md w-full animate-scale-in shadow-elevated">
            <button onClick={() => setShowAddModal(false)} className="absolute top-4 right-4 p-2 rounded-xl hover:bg-atlas-bg-subtle transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            <h2 className="text-xl font-semibold mb-1 tracking-tight">Add recipient</h2>
            <p className="text-sm text-atlas-text-secondary mb-6">Enter their bank details.</p>
            <form onSubmit={handleAdd} className="space-y-4 max-h-[65vh] overflow-y-auto pr-1 -mr-1">
              ...
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-atlas-text-secondary">Country (optional)</label>
              ...
              <div className="space-y-1.5">
                <label htmlFor="add-notes" className="block text-sm font-medium text-atlas-text-secondary">Notes (optional)</label>
                <textarea
                  id="add-notes"
                  rows={3}
                  ...
                  className="flex w-full rounded-xl border border-atlas-border bg-white px-4 py-3 text-sm text-atlas-text-primary ring-offset-background transition-colors placeholder:text-atlas-text-secondary/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0 resize-none"
                />
              </div>
      ...
      {editingRecipient && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl p-8 max-w-md w-full animate-scale-in shadow-elevated">
            <button onClick={() => setEditingRecipient(null)} className="absolute top-4 right-4 p-2 rounded-xl hover:bg-atlas-bg-subtle transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            <h2 className="text-xl font-semibold mb-1 tracking-tight">Edit recipient</h2>
            <p className="text-sm text-atlas-text-secondary mb-6">Update their details.</p>
            <form onSubmit={handleEditSave} className="space-y-4 max-h-[65vh] overflow-y-auto pr-1 -mr-1">
              ...
              <div>
                <label className="text-sm font-medium text-atlas-text-secondary mb-1.5 block">IBAN</label>
                <p className="text-sm font-mono text-atlas-text-primary bg-atlas-bg-subtle/50 rounded-xl px-4 py-3">{formatIban(editingRecipient.iban)}</p>
              </div>
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-atlas-text-secondary">Country</label>
              ...
              <div className="space-y-1.5">
                <label htmlFor="edit-notes" className="block text-sm font-medium text-atlas-text-secondary">Notes</label>
                <textarea
                  id="edit-notes"
                  rows={3}
                  placeholder="Add a note about this recipient…"
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        isSelected ? 'bg-atlas-accent/5' : 'hover:bg-atlas-bg-subtle/30',
        !isLast && 'border-b border-atlas-border/50',
      )}
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          {recipient.isFavorite && (
            <svg className="w-3 h-3 text-atlas-black fill-atlas-black flex-shrink-0" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
            </svg>
          )}
        </div>
        <p className="mt-0.5 truncate text-xs text-atlas-text-secondary">
          {recipient.bank ? `${recipient.bank} · ` : ''}{recipient.lastTransfer}
        </p>
      </div>
      <svg className="w-4 h-4 text-atlas-text-secondary/40 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function TransactionRow({ transaction, isLast }: { transaction: Transaction; isLast: boolean }) {
  ...
  return (
    <Link
      ...
      className={clsx(
        'group flex items-center gap-3.5 px-5 py-3.5 transition-colors hover:bg-atlas-bg-subtle/40',
        !isLast && 'border-b border-atlas-border/40',
      )}
    >
      <div className={clsx(
        'w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0',
        isOutbound ? 'bg-atlas-error/10' : 'bg-atlas-green-50',
      )}>
        <svg className={clsx('w-3.5 h-3.5', isOutbound ? 'text-atlas-error rotate-45' : 'text-atlas-black -rotate-[135deg]')} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" />
        </svg>
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium truncate">{transaction.reference || (isOutbound ? 'Sent' : 'Received')}</p>
        <p className="text-xs text-atlas-text-secondary mt-0.5">{formatRelative(transaction.createdAt)}</p>
      </div>
      <div className="text-right flex-shrink-0">
        <p className={clsx('text-sm font-medium tabular-nums', isOutbound ? 'text-atlas-text-primary' : 'text-atlas-success')}>
          {isOutbound ? '-' : '+'}{formatMoney(transaction.amountCents, transaction.currency)}
        </p>
        <p className={clsx('text-[11px] mt-0.5 capitalize', transaction.status === 'pending' ? 'text-atlas-accent-700' : 'text-atlas-text-secondary')}>
          {transaction.status}
        </p>
      </div>
      <svg className="w-4 h-4 text-atlas-text-secondary/30 flex-shrink-0 transition-colors group-hover:text-atlas-text-secondary/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
      </svg>
    </Link>
  );
...
function DetailRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex justify-between items-center py-3 border-b border-atlas-border/40 last:border-0">
      ...
      <span className={clsx('text-sm font-medium text-atlas-text-primary text-right', mono && 'font-mono text-xs tracking-wide')}>{value}</span>
    </div>
  );
}
