...
function RowSkeleton() {
  return (
    <div className="flex items-center gap-3 py-3 px-3 animate-pulse">
      <div className="w-10 h-10 rounded-full bg-atlas-bg-subtle" />
      <div className="flex-1 space-y-1.5">
        <div className="h-3 bg-atlas-bg-subtle rounded w-36" />
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 max-w-3xl mx-auto">
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-sm text-atlas-text-secondary hover:text-atlas-text-primary transition-colors mb-5"
      >
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        ...
        <div className="text-right">
          <p className="text-[11px] uppercase tracking-wider text-atlas-text-secondary">Spent</p>
          ...
          {cards.map((c) => (
            <button
              ...
              className={clsx(
                ...
                c.id === activeCardId
                  ? 'border-atlas-text-primary/20 bg-white shadow-card text-atlas-text-primary'
                  : 'border-transparent bg-atlas-bg-subtle/60 hover:bg-atlas-bg-subtle text-atlas-text-secondary'
              )}
            >
              {c.label} · {c.last4}
            </button>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
          ...
          {(['all', 'in', 'out'] as const).map((tab) => (
            <button
              ...
              className={clsx(
                ...
                filter === tab ? 'bg-white text-atlas-text-primary shadow-sm' : 'text-atlas-text-secondary hover:text-atlas-text-primary'
              )}
            >
        ...
        <div className="relative flex-1">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-atlas-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-xl border border-atlas-border text-sm focus:outline-none focus:border-atlas-accent transition-colors"
          />
        </div>
      </div>

      {/* List */}
      <div className="bg-white rounded-2xl border border-atlas-border/70 p-4 sm:p-6">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-atlas-bg-subtle">
          ...
          <>
            {grouped.map(([date, txs]) => (
              <div key={date} className="mb-6 last:mb-0">
                <p className="text-[12px] font-medium uppercase tracking-wider text-atlas-text-secondary mb-2">{date}</p>
                <div className="space-y-0.5">
                  {txs.map((tx) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center gap-3 py-3 px-3 -mx-3 rounded-xl hover:bg-atlas-bg-subtle/50 transition-colors text-left"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-atlas-text-secondary truncate">{tx.reference || formatCategory(tx.category)}</p>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <p className={clsx('text-sm font-semibold tabular-nums', inbound ? 'text-atlas-success' : 'text-atlas-text-primary')}>
