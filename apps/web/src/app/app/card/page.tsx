...
function MiniCardSymbol({ card, active }: { card: CardData; active: boolean }) {
  ...
  const ink = light ? 'text-auriga-text-primary/60' : 'text-white/55';
  return (
    <div
      className={clsx(
    ...
    >
      ...
      {isSingleUse ? (
        ...
        <svg className={clsx('absolute top-[6px] right-[7px] w-[9px] h-[9px]', light ? 'text-auriga-text-primary/40' : 'text-white/35')} viewBox="0 0 24 24" fill="none">
...
const TYPE_META: Record<CardData['type'], { label: string; color: string; dot: string }> = {
  virtual: { label: 'Virtual', color: 'bg-auriga-heather-100 text-auriga-heather-600', dot: 'bg-auriga-heather-400' },
  physical: { label: 'Physical', color: 'bg-auriga-heather-100 text-auriga-heather-600', dot: 'bg-auriga-heather-500' },
  'single-use': { label: 'Single-use', color: 'bg-auriga-heather-100 text-auriga-heather-600', dot: 'bg-auriga-heather-400' },
};
...
const CARD_THEMES: Record<CardColor, {
  ...
  black: {
    ...
    miniBg: 'bg-auriga-black',
  },
  white: {
    label: 'Arctic',
    bg: 'bg-gradient-to-br from-white via-[#FBFAF7] to-[#EDEAE3]',
    light: true,
    swatch: 'bg-white ring-1 ring-inset ring-auriga-border',
    ...
    <div className="p-4 sm:p-6 lg:p-10 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-auriga-text-secondary mt-1">{cards.length} card{cards.length !== 1 ? 's' : ''} active</p>
        </div>
      ...
      <div className="mb-8">
        ...
        >
            ...
            return (
              <button
                ...
                className={clsx(
                  ...
                  isActive
                    ? 'bg-white shadow-card ring-1 ring-auriga-text-primary/10'
                    : 'bg-auriga-bg-subtle/50 hover:bg-auriga-bg-subtle'
                )}
              >
                {/* Active accent rail */}
                <span className={clsx(
                  'absolute left-0 top-1/2 -translate-y-1/2 w-[3px] rounded-full bg-auriga-accent transition-all duration-300',
                ...
                <div className="text-left">
                  {isEditing && !isActive ? (
                    <input
                      ...
                      className="text-[13px] font-semibold leading-tight bg-transparent border-b border-auriga-accent outline-none w-24 py-0"
                      maxLength={40}
                      autoFocus
                    />
                  ) : (
                    <p
                      ...
                      className={clsx(
                        ...
                        isActive ? 'text-auriga-text-primary' : 'text-auriga-text-secondary'
                      )}
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    <div className={clsx('w-1.5 h-1.5 rounded-full', c.status === 'frozen' ? 'bg-auriga-text-secondary' : meta.dot)} />
                    <span className="text-[10px] text-auriga-text-secondary capitalize whitespace-nowrap">
          ...
          <button
            ...
            className="group flex-shrink-0 flex items-center gap-3 pl-3 pr-5 py-2.5 rounded-2xl border border-dashed border-auriga-border hover:border-auriga-accent/50 hover:bg-auriga-bg-subtle/40 transition-all"
          >
            <span className="relative w-[62px] h-[39px] rounded-[7px] flex items-center justify-center bg-auriga-bg-subtle border border-dashed border-auriga-border/80 group-hover:border-auriga-accent/40 transition-colors">
              <svg className="w-4 h-4 text-auriga-text-secondary group-hover:text-auriga-text-primary transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
              </svg>
            </span>
            <span className="text-[13px] font-semibold text-auriga-text-secondary group-hover:text-auriga-text-primary transition-colors whitespace-nowrap">Add card</span>
          </button>
        </div>
      </div>

      {cards.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center py-20 px-6 bg-white rounded-2xl border border-auriga-border/70">
          <div className="w-16 h-16 rounded-2xl bg-auriga-bg-subtle flex items-center justify-center mb-5">
            <svg className="w-8 h-8 text-auriga-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
            </svg>
          </div>
          <h2 className="text-lg font-semibold tracking-tight">No cards yet</h2>
          <p className="text-sm text-auriga-text-secondary mt-1 max-w-xs">You've removed all your cards. Create a new virtual, physical, or single-use card to get started.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-10">
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="flex flex-col items-center lg:items-start">
              ...
              >
                ...
                <div className="relative z-[3] h-full flex flex-col justify-between p-7 sm:p-9">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2.5">
                      <img
                        src={isLightCard ? '/auriga-lockup.svg' : '/auriga-lockup-light.svg'}
                        alt="Auriga"
                        className={clsx('h-6 w-auto', isLightCard ? 'opacity-90' : 'opacity-[0.92]')}
                      />
                    </div>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className={isLightCard ? 'text-auriga-text-secondary mt-0.5' : 'text-white/20 mt-0.5'}>
                  ...
                  {activeCard.type === 'single-use' && (
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full bg-auriga-text-secondary/40" />
                      <span className="text-[10px] uppercase tracking-[0.15em] text-auriga-text-secondary font-medium">One-time use</span>
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                    {showDetails ? (
                      <p className={clsx(
                        ...
                        isLightCard ? 'text-auriga-black' : 'text-white/80'
                      )}>
                      ...
                      <p className={clsx(
                        ...
                        isLightCard ? 'text-auriga-text-secondary' : 'text-white/40'
                      )}>
                        •••• •••• •••• <span className={isLightCard ? 'text-auriga-black' : 'text-white/70'}>{activeCard.last4}</span>
                      </p>
                    )}
                    ...
                    <div className="flex items-end justify-between">
                      <div>
                        <p className={clsx(
                          ...
                          isLightCard ? 'text-auriga-text-secondary' : 'text-white/20'
                        )}>Cardholder</p>
                        <p className={clsx(
                          'text-[12px] sm:text-[13px] font-medium tracking-[0.06em]',
                          isLightCard ? 'text-auriga-black/80' : 'text-white/70'
                        )}>
                ...
                {frozen && (
                  <div className="absolute inset-0 z-[6] flex items-center justify-center">
                    <div className="absolute inset-0 bg-auriga-bg-subtle/80" />
                    <div className="absolute inset-0 backdrop-blur-[2px]" />
                    <div className="relative flex flex-col items-center gap-3">
                      <div className="w-12 h-12 rounded-full bg-auriga-heather-100 border border-auriga-border flex items-center justify-center">
                      ...
                      <div className="flex items-center gap-2 bg-white/90 backdrop-blur-xl rounded-full px-5 py-2 border border-auriga-border">
                        <div className="w-2 h-2 rounded-full bg-auriga-text-secondary animate-pulse" />
                        <span className="text-[12px] font-semibold text-auriga-black tracking-[0.12em] uppercase">Card Frozen</span>
                      </div>
                      <p className="text-[10px] text-auriga-text-secondary mt-0.5">Unfreeze to resume transactions</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="mt-5 flex items-center gap-2">
              <div className={clsx(
                ...
                frozen ? 'bg-auriga-text-secondary' : TYPE_META[activeCard.type].dot
              )} />
              {editingCardId === activeCardId ? (
                <input
                  ...
                  className="text-xs bg-transparent border-b border-auriga-accent outline-none py-0"
                  maxLength={40}
                  autoFocus
                />
              ) : (
                <span
                  className="text-xs text-auriga-text-secondary cursor-text"
            ...
            <div className="mt-5 flex items-center gap-3">
                  ...
                  return (
                    <button
                      ...
                      className={clsx(
                        ...
                        selected
                          ? 'ring-2 ring-offset-2 ring-offset-white ring-auriga-text-primary/50 scale-110 shadow-sm'
                      ...
                      {selected && (
                        <svg
                          className={clsx(
                            ...
                            CARD_THEMES[col].light ? 'text-auriga-text-primary' : 'text-white'
                          )}
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
              ...
              <span className="text-xs text-auriga-text-secondary">{CARD_THEMES[activeCard.color].label}</span>
            </div>
          </div>
            ...
            {cardControls.map((ctrl) => (
              <button
                ...
                className={clsx(
                  ...
                  'bg-auriga-bg-subtle/60 hover:bg-auriga-bg-subtle'
                )}
              >
                <div className="text-auriga-text-secondary group-hover:text-auriga-text-primary transition-colors duration-200">
                  {ctrl.icon}
                </div>
                <span className="text-[11px] font-medium text-auriga-text-secondary group-hover:text-auriga-text-primary transition-colors">
                  {ctrl.label}
                </span>
              </button>
            ))}
          ...
          <div className="bg-white rounded-2xl border border-auriga-border/70 p-6">
            <h2 className="text-[13px] font-semibold mb-5 uppercase tracking-wider text-auriga-text-secondary">Settings</h2>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-auriga-border/70 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[13px] font-semibold uppercase tracking-wider text-auriga-text-secondary">Recent</h3>
              <Link
                href={`/app/card/transactions?cardId=${activeCardId}` as any}
                className="text-xs text-auriga-text-secondary hover:text-auriga-text-primary transition-colors"
              >
                View all
              </Link>
            </div>
                ...
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="flex items-center gap-3 py-3 px-2 animate-pulse">
                    ...
                    <div className="flex-1 space-y-1.5">
                      <div className="h-3 bg-auriga-bg-subtle rounded w-28" />
                      <div className="h-2.5 bg-auriga-bg-subtle rounded w-16" />
                    </div>
                    <div className="h-3 bg-auriga-bg-subtle rounded w-12" />
                  </div>
                ))}
              </div>
            ) : recentTransactions.length === 0 ? (
              <div className="py-10 text-center">
                <p className="text-sm text-auriga-text-secondary">No transactions yet</p>
                <p className="text-xs text-auriga-text-secondary/70 mt-1">Spending on this card will show up here.</p>
              </div>
            ) : (
              <div className="space-y-0.5">
                {recentTransactions.map((tx) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center gap-3 py-3 px-2 -mx-2 rounded-xl hover:bg-auriga-bg-subtle/50 transition-colors cursor-pointer text-left"
                    >
                      <MerchantLogo name={tx.counterpartyName} size={36} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{tx.counterpartyName}</p>
                        <p className="text-[11px] text-auriga-text-secondary truncate">{tx.reference || formatCategory(tx.category)}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <span className={clsx('text-sm font-medium tabular-nums', inbound ? 'text-auriga-success' : 'text-auriga-text-primary')}>
                          {inbound ? '+' : '−'}€{(tx.amountCents / 100).toFixed(2)}
                        </span>
                        <p className="text-[10px] text-auriga-text-secondary">{relativeDate(tx.createdAt)}</p>
                      </div>
                    </button>
                  );
                })}
          ...
          {/* All cards overview */}
          <div className="bg-white rounded-2xl border border-auriga-border/70 p-6">
            <h3 className="text-[13px] font-semibold mb-4 uppercase tracking-wider text-auriga-text-secondary">All cards</h3>
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      ...
                      c.id === activeCardId ? 'bg-auriga-bg-subtle' : 'hover:bg-auriga-bg-subtle/50'
                    )}
                  >
                    <div className="w-10 h-7 rounded-md flex items-center justify-center text-[7px] font-bold tracking-wider flex-shrink-0 bg-auriga-bg-subtle text-auriga-text-secondary border border-auriga-border/60">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        {c.status === 'frozen' && (
                          <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-md bg-auriga-accent/15 text-auriga-accent-700">Frozen</span>
                        )}
                      </div>
                    </div>
                    <p className="text-xs text-auriga-text-secondary tabular-nums flex-shrink-0">
                      •••• {c.last4}
                    </p>
                  </button>
                );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl p-8 max-w-md w-full animate-scale-in shadow-elevated">
            <button
              ...
              className="absolute top-4 right-4 p-2 rounded-xl hover:bg-auriga-bg-subtle transition-colors"
            >
            ...
            {physicalStep === 'select' ? (
              <>
                ...
                <p className="text-sm text-auriga-text-secondary mb-8">Choose your card type</p>

                <div className="space-y-4">
                  {/* Virtual Card */}
                  <button
                    onClick={() => createCard('virtual')}
                    className="group w-full rounded-2xl border border-auriga-border/60 hover:border-auriga-border transition-all duration-300 overflow-hidden text-left hover:shadow-lg hover:shadow-auriga-heather-200/40"
                  >
                    <div className="relative p-5">
                      <div className="absolute inset-0 bg-auriga-bg-subtle opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                      <div className="relative flex items-center gap-4">
                        {/* Mini card visual */}
                        <div className="w-[72px] h-[46px] rounded-lg bg-auriga-black flex-shrink-0 relative overflow-hidden shadow-md group-hover:scale-105 transition-transform duration-300 border border-white/10">
                          ...
                          <div className="absolute top-2 right-2">
                            <svg className="w-2.5 h-2.5 text-auriga-text-secondary/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-semibold group-hover:text-auriga-black transition-colors duration-300">Virtual</p>
                            <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-auriga-heather-100 text-auriga-heather-600 group-hover:bg-auriga-heather-200 group-hover:text-auriga-heather-700 transition-colors duration-300">Instant</span>
                          </div>
                          <p className="text-xs text-auriga-text-secondary group-hover:text-auriga-text-secondary transition-colors duration-300 mt-1">
                            Ready in seconds. Perfect for online payments & subscriptions.
                          </p>
                        </div>
                        <svg className="w-5 h-5 text-auriga-text-secondary/40 group-hover:text-auriga-text-secondary group-hover:translate-x-0.5 transition-all duration-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  ...
                  >
                    <div className="relative p-5">
                      ...
                      <div className="relative flex items-center gap-4">
                        {/* Mini card visual */}
                        <div className="w-[72px] h-[46px] rounded-lg bg-auriga-bg-subtle flex-shrink-0 relative overflow-hidden shadow-md group-hover:scale-105 transition-transform duration-300 border border-auriga-border/60">
                          <div className="absolute top-2 left-2.5">
                            <div className="w-5 h-3.5 rounded-[2px] bg-auriga-heather-300/30 border border-auriga-heather-400/20" />
                          </div>
                          <div className="absolute top-2 right-2">
                            <span className="text-[5px] uppercase tracking-widest text-auriga-text-secondary/60 font-bold">Metal</span>
                          </div>
                          <div className="absolute bottom-2 left-2.5 flex gap-[3px]">
                            <div className="w-1 h-1 rounded-full bg-auriga-text-secondary/30" />
                            <div className="w-1 h-1 rounded-full bg-auriga-text-secondary/30" />
                            <div className="w-1 h-1 rounded-full bg-auriga-text-secondary/30" />
                            <div className="w-1 h-1 rounded-full bg-auriga-text-secondary/30" />
                          </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-semibold group-hover:text-auriga-black transition-colors duration-300">Physical</p>
                            <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-auriga-heather-100 text-auriga-heather-600 group-hover:bg-auriga-heather-200 group-hover:text-auriga-heather-700 transition-colors duration-300">Premium</span>
                          </div>
                          <p className="text-xs text-auriga-text-secondary group-hover:text-auriga-text-secondary transition-colors duration-300 mt-1">
                      ...
                      <div className="relative flex items-center gap-4">
                        ...
                        <div className="w-[72px] h-[46px] rounded-lg bg-auriga-bg-subtle flex-shrink-0 relative overflow-hidden shadow-md group-hover:scale-105 transition-transform duration-300 border border-auriga-border/60">
                          <div className="absolute top-2 left-2.5 flex items-center gap-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-auriga-text-secondary/40" />
                            <span className="text-[5px] text-auriga-text-secondary/60 uppercase tracking-wider font-medium">1x</span>
                          </div>
                          <div className="absolute bottom-2 left-2.5 flex gap-[3px]">
                            <div className="w-1 h-1 rounded-full bg-auriga-text-secondary/30" />
                            <div className="w-1 h-1 rounded-full bg-auriga-text-secondary/30" />
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-semibold group-hover:text-auriga-black transition-colors duration-300">Single-use</p>
                            <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-auriga-heather-100 text-auriga-heather-600 group-hover:bg-auriga-heather-200 group-hover:text-auriga-heather-700 transition-colors duration-300">Secure</span>
                          </div>
                          <p className="text-xs text-auriga-text-secondary group-hover:text-auriga-text-secondary transition-colors duration-300 mt-1">
              ...
              <>
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-auriga-text-secondary hover:text-auriga-text-primary transition-colors mb-4"
                >
                ...
                <p className="text-sm text-auriga-text-secondary mb-6">Where should we send your physical card?</p>
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    !useNewAddress
                      ? 'border-auriga-accent bg-auriga-accent/5'
                      : 'border-auriga-border hover:border-auriga-text-secondary/30'
                  )}
                >
                  <div className="flex items-center gap-3">
                    <div className={clsx(
                      'w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0',
                      !useNewAddress ? 'border-auriga-accent' : 'border-auriga-border'
                    )}>
                      {!useNewAddress && <div className="w-2.5 h-2.5 rounded-full bg-auriga-accent" />}
                    </div>
                    <div>
                ...
                {useNewAddress && (
                  <div className="mt-4 space-y-3 animate-fade-in">
                    <div>
                      <label className="text-[11px] font-medium text-auriga-text-secondary uppercase tracking-wider">Address line 1</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-[11px] font-medium text-auriga-text-secondary uppercase tracking-wider">Address line 2 (optional)</label>
                      <input
                        type="text"
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div className="min-w-0">
        ...
        <p className="text-[11px] text-auriga-text-secondary">{description}</p>
      </div>
      <button
        role="switch"
        ...
        className={clsx(
          ...
          checked ? 'bg-auriga-accent' : 'bg-auriga-border'
        )}
      >
        <div className={clsx(
