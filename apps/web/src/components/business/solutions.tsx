      ...
      <Container>
        <Reveal as="div" className="max-w-3xl mb-12 sm:mb-16">
          ...
          <p className="mt-5 text-lg sm:text-xl text-auriga-text-secondary leading-relaxed">
            ...
            borders, Auriga Business adapts to the way you operate.
          </p>
        </Reveal>
        ...
        <div className="grid md:grid-cols-3 gap-4 sm:gap-5">
          {solutions.map((solution, i) => (
            <Reveal
              key={solution.title}
              delay={i * 90}
              className="rounded-3xl bg-white border border-auriga-border/70 p-7 sm:p-8 flex flex-col"
            >
              <span className="text-[11px] font-bold uppercase tracking-widest text-auriga-text-secondary">
                {solution.badge}
              </span>
              <h3 className="mt-3 text-xl font-bold text-auriga-text-primary tracking-tight">{solution.title}</h3>
              <p className="mt-2 text-[15px] text-auriga-text-secondary leading-relaxed">{solution.description}</p>
            </Reveal>
          ))}
        </div>
      </Container>
