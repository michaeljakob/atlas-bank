...
export function BusinessCTA() {
  return (
    <section className="py-24 sm:py-36 bg-white">
      <Container>
        <Reveal direction="scale" className="relative bg-auriga-dark-surface rounded-[2.5rem] px-8 py-20 sm:px-16 sm:py-28 overflow-hidden">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div className="relative">
              <h2 className="text-4xl sm:text-6xl lg:text-7xl text-white font-heading uppercase tracking-tight leading-[0.92]">
                Open your
                <br />
                <span className="text-auriga-accent">business account.</span>
              </h2>
              <p className="mt-8 text-lg sm:text-xl text-auriga-heather-300 max-w-md leading-relaxed">
                ...
                {['EUR IBAN', 'Team cards', 'Batch payments', 'SEPA Instant', 'Accounting sync', 'Role-based access'].map((feature) => (
                  <div key={feature} className="flex items-center gap-2.5 rounded-2xl bg-white/5 px-4 py-3.5 ring-1 ring-white/10">
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-auriga-accent/20 shrink-0">
                      <svg className="w-3 h-3 text-auriga-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
